...
function g_init(floor){
	...
	var bildnum=gs_getBuildNum(floor);
...
function g_changeFloor(floor){
	if(gs_getMapBounds(gs_getBuildNum(floor))==null){
		alert("GIS地图不存在！");
		return ;
	}
	if(g_map!=null){
	...
	if(floor!="Z"){
		...
		//light.featureSelect=function(id){alert(id)};
		//light.featureSelect=gc_clickDevice;
		light.activate();
	}else{		
		g_map=new g_tilertmap("G_Map",OlURL+"/tiler/");
		g_map.click=function(id){alert("选中楼id："+id);}
	}
...
function gs_getFeaturesById(hpid,flag,callback){
	var layers=gs_getTypeNameByHpid(hpid);
	if(layers!=""){
		var filter="hpid='"+hpid+"'";
		gs_query(layers,filter,function(features){
				if(features.length>0){
					callback(features[0],flag);
				}});
	}
	layers=null;
	//CollectGarbage();
...
function refreshCarparkStatus(){
	var chekulayers=g_map.getLayersByName("ceng_PMS");
...
function g_getLayerName(){
	...
	if(layers!=null){	
		for(var i=0;i<layers.length;i++){
			if(OpenLayers.String.contains(layers[i].name,"ceng_")&&layers[i].visibility==true&&!OpenLayers.String.contains(layers[i].name,"_mk")){
...
function gs_zoomChanged(evt){
	...
	if(gs_statearr&&gs_statearr.length>0){
		for(var i=0;i<gs_statearr.length;i++){
			...
			var showlevel=gs_deviceShowLevel.get(hpid);
			var layername=gs_getTypeById(hpid);
			if(gs_markerLayerExist(layername)==true){
				var mklayer=g_map.getLayersByName(layername+"_mk")[0];
				var marker=gs_markermap.get(hpid);
				if(marker){
