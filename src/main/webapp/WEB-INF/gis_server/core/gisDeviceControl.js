...
function gs_addalarm(array){
	...
	for(var i=0;i<array.length;i++){
		...
		if(OpenLayers.String.contains(hpid,g_selectfloor)){
			gs_getFeaturesById(hpid,0,gs_getFeaturesByIdCallBack);  //获得hpid，ajax执行回调函数
		}
	}
	array=null;
}
...
function gs_addstate(array){
	...
	for(var i=0;i<arr.length;i++){
		...
		if(OpenLayers.String.contains(hpid,g_selectfloor)){
			gs_getFeaturesById(OpenLayers.String.trim(arr[i]),2,gs_getFeaturesByIdCallBack);  //获得hpid，ajax执行回调函数
		}
	}
	arr=null;
}
...
function gs_removePointstate(deviceids){
	for(var i=0;i<deviceids.length;i++){
		var layername=gs_getTypeById(OpenLayers.String.trim(deviceids[i]));
		gs_removeAlarmMarker(layername,OpenLayers.String.trim(deviceids[i]));
		layername=null;
	}
}
 ...
 * deviceids 数组
 */
function gs_removePointAlarm(deviceids){
	for(var i=0;i<deviceids.length;i++){
		var layername=gs_getTypeById(OpenLayers.String.trim(deviceids[i]));
		gs_removeAlarmMarker(layername,OpenLayers.String.trim(deviceids[i]));
		gs_removeAlarmPop(OpenLayers.String.trim(deviceids[i]));
		layername=null;
	}
}
