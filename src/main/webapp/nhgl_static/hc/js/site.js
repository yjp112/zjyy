...
function loadUrl(url, defaultUrl,callback) {
    $("#_loading").show();
    if (!url) return;
    ...
    $("#centerMain").load(url, function(responseText, textStatus, xhr) {
        ...
        if (xhr.status == 200) {
            ...
            //     path: '/'
            // });
			if(callback && jQuery.isFunction(callback)){callback(responseText, textStatus, xhr);}
        } else {
            if (defaultUrl) {
                //clearContentLayouts();
                loadUrl(defaultUrl,  null,callback);
            }
        }
    });
}
...
(function($) {
     ...
     * 通用能耗选部门组件
     */
    $.selectNHDepartment = function(_this, options) {
        _common_select(_this, "/nhgl/basic/ngDept/select", "_select_department", "选择部门", 350, 500, function(_opt) {
            var selected;
            if (_opt.muilt) {
                selected = plat_org_department_select_tree.getCheckedNodes(true);
            } else {
