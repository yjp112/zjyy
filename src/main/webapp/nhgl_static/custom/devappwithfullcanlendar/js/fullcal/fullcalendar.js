...
function Calendar(element, options, eventSources) {
	...
	monn=options.m;
	dayn=options.d;
	var arr = options.defaultDate.split("-");
	...
	function renderView(inc) {
		if (elementVisible()) {
			...
			var today = new Date(yearn,monn,dayn);
...
function BasicView(element, calendar, viewName) {
	...
	function renderBasic(r, c, showNumbers) {
		...
		var today = clearTime(new Date(yearn,monn,dayn));
...
function AgendaView(element, calendar, viewName) {
	...
	function renderAgenda(c) {
		...
		var d0 = rtl ? addDays(cloneDate(t.visEnd), -1) : cloneDate(t.visStart),
			d = cloneDate(d0),
			today = clearTime(yearn,monn,dayn),
