...
**/
function quick_dialog(id,title,url,width,height,btn,reload,closeCallback){
	if(window != top.window){
		top.quick_dialog(id,title,url,width,height,btn,reload,closeCallback);
		return;
	}
...
function quick_dialog_ref(id,title,url,width,height,btn,reload){
	if(window != top.window){
		top.quick_dialog_ref(id,title,url,width,height,btn);
		return;
	}
...
function quick_datagrid(el, formId, readUrl, columns, options, dsOptions, operates, toolbarId) {
	...
	$.each(columns,function(idx,it){
		$.extend(true,it,{sortable: true});
		//console.log(idx)
		//console.log(it)
	});	

	_dbopt.serverSorting=false;
	//console.log(_dbopt)
	//========add by dyg===========
    var _opt = {
...
function btn_running(btn) {
	...
	if (btn.tagName.toUpperCase() == "A") {
		...
		$(btn).addClass("disabled").data("text", $(btn).html());
