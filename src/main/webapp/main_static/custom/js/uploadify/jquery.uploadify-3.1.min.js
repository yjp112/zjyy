...
(function(c) {
					...
					.each(function() {
						...
						if (h) {
							...
							i.queueData = {
								...
								uploadQueue : [],
								errorMsg : "上传失败："//"Some files were not added to the queue:"
							};
							i.original = m;
							i.wrapper = l;
							i.button = f;
	...
	var a = {
		onDialogOpen : function() {
			var d = this.settings;
			this.queueData.errorMsg = "上传失败：";//"Some files were not added to the queue:";
		...
		onSelectError : function(d, g, f) {
			...
			if (c.inArray("onSelectError", e.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					/*this.queueData.errorMsg += '\n这个 "' + d.name
							+ '" 超过了大小限制 (' + e.fileSizeLimit
							+ ").";*/
					this.queueData.errorMsg += '\n对不起,您上传的文件 "' + d.name
					+ '" 过大，文件大小请限制在 (' + e.fileSizeLimit
					+ ").";
					break;
