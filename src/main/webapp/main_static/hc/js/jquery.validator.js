...
(function($, undefined) {
    ...
    Validator.prototype = {
        ...
        _validatedField: function(el, field, ret) {
            ...
            // kendo combobox 组件
            if($(el).attr("data-role") == "combobox") {
                $(el).parent(".k-combobox").removeClass( isValid ? opt.invalidClass : opt.validClass )
                                           .addClass( !ret.skip ? isValid ? opt.validClass : opt.invalidClass : "" )
                                           .trigger( callback + CLS_NS_FIELD, [ret, me] );
            }
        ...
        mapMsg: function(obj) {
            ...
            $.each(obj, function(name, msg) {
                ...
                $(el).addClass(me.options.invalidClass);
                me.showMsg(el, msg);
            });
        },
    ...
    $(document)
        ...
        .on('mouseover', 'form input, form .k-textbox', function(e){
            if ($(this).hasClass("n-invalid")) {
                var msg_box = $(this).siblings(".msg-box").get(0);
                $(msg_box).find(".msg-wrap").show();
            }
            // kendo combobox 组件
            else if($(this).attr("role") == "combobox") {
                if($(this).parent("span").siblings('.n-invalid').length > 0) {
                    var msg_box = $(this).parent("span").siblings('.msg-box').get(0);
                    $(msg_box).find(".msg-wrap").show();
                }
            }
        })

        .on('mouseout', 'form input, form .k-textbox', function(e){
            if ($(this).hasClass("n-invalid")) {
                $(this).siblings(".msg-box").find(".msg-wrap").hide();
            }
            // kendo combobox 组件
            else if($(this).attr("role") == "combobox") {
                if($(this).parent("span").siblings('.n-invalid').length > 0) {
                    $(this).parent("span").siblings('.msg-box').find(".msg-wrap").hide();
                }
            }
        })
...
(function(factory) {
    ...
    $.validator.config({
        ...
        rules: {
            ...
            ,url: [/^[a-zA-z]+:\/\/[^\s]*/, "网址格式不正确"]
            ,postcode: [/^[1-9]\d{5}$/, "邮政编码格式不正确"]
            ,chinese: [/^[\u0391-\uFFE5]+$/, "请输入中文"]
            ,username: [/^\w{3,12}$/, "请输入3-12位数字、字母、下划线"]
            ,password: [/^[0-9a-zA-Z]{6,16}$/, "密码由6-16位数字、字母组成"]
            ,double: [/^[-+]?\d+(\.\d+)?$/, "请输入正确的实数"]
            ,name: [/^\w{0,32}$/, "请输入少于32位数字、字母、下划线"]
            ,path: [/^(\.|\/|[a-zA-Z])?:?\/.+(\/$)?/, "路径格式不正确"]
            //,winpath: [/^[a-zA-Z];[\\/]((?! )(?![^\\/]*\s+[\\/])[\w -]+[\\/])*(?! )(?![^.]+\s+\.)[\w -]+$/, "路径格式不正确"]
            //,lnxPath: [/^([\/] [\w-]+)*$/, "路径格式不正确"]
