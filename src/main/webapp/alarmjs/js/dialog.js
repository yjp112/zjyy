(function($, window, undefined) {
    ...
    var _ie6 = window.ActiveXObject && !window.XMLHttpRequest,
        ...
        dialogTpl = '<table class="ui_border">'
            + '<tbody>'
            + '<tr>'
            + '<td class="ui_lt"></td>'
            + '<td class="ui_t"></td>'
    ...
    var lhgdialog = dialog = function(config) {
        ...
        return lhgdialog.list[config.id] = _singleton ? _singleton._init(config) : new lhgdialog.fn._init(config);
    };
    ...
    lhgdialog.fn = lhgdialog.prototype = {
        ...
        _init: function(config) {
            ...
            that.title(config.title).content(content, true, isIfr, isLoad).size(config.width, config.height).position(config.left, config.top).time(config.time)[config.show ? 'show': 'hide'](true).zindex();
      ...
      focus: true
        ...
        button: function() {
            ...
            for (; i < ags.length; i++) {
                ...
                if (item.callback) listeners[id].callback = item.callback;
         ...
         * @return  {this}    如果无参数则返回对象本身
         */
        content: function(msg, add, frm, load) {
