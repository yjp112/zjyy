...
public class OrganizationUtils {
	...
	private static String group = Organization.class.getName();
	private static ReentrantReadWriteLock rwl = new ReentrantReadWriteLock();

	private static OrganizationService service = SpringContextHolder.getBean(OrganizationService.class);
	...
	public static	List<Organization> getFullDeptNameByPersonId(long personId){
		rwl.readLock().lock();
		try {
			List<Organization> result = cache.get(group, String.valueOf(personId));
			if (result == null) {
				rwl.readLock().unlock();
				rwl.writeLock().lock();
				try {
	...
	public static String joinFullDeptName(List<Organization> organizations){
