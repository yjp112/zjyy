 ...
 */
//@WebServlet(loadOnStartup = 0, urlPatterns = { "/startupBeforeSpring" })
public class StartupLoaderBeforeSpringServlet extends HttpServlet {
	 ...
	 * 异步启动，多线程启动，多个插件同时启动
	 */
	private List<IStartupPlugin> noOrderstartupPlugins = new ArrayList<IStartupPlugin>();
	...
	public StartupLoaderBeforeSpringServlet() {
	}

	public void init(ServletConfig config) throws ServletException {
		threadLocal.set(config);
		PropertiesLoader loader = new PropertiesLoader(
				"classpath:startup-beforeLoadSpring-plugins.properties");
		for (Object s : loader.getProperties().keySet()) {
			...
			if(key.startsWith(PREFIX_ORDER)){
				...
				instanceClass(noOrderstartupPlugins, value);
			}
		}
		for (IStartupPlugin plugin : orderStartupPlugins) {
			plugin.startup();
		}
		if (noOrderstartupPlugins.size() > 0) {
			ExecutorService executorService = Executors.newCachedThreadPool();
			for (final IStartupPlugin plugin : noOrderstartupPlugins) {
