package com.supconit.nhgl.analyse.gas.dept.controller;
 ...
 *
 */
@Controller("DeptGasCountController")
@RequestMapping("nhgl/analyse/gas/dept")
public class DeptCountController {
	...
	public String index(ModelMap model, Integer year, String flag, String fxCode, String menuName,String code,String deptId) {
		...
		nhlst=nhQDeptMonthService.getDeptList(nh);
		...
		return "nhgl/analyse/gas/dept/dept_count_tab";
	}
	...
	public String allYearData(ModelMap model, String deptName, Integer year,String fxCode,
		...
		nh.setStartTime(year+"-01");
		nh.setNhType(Constant.NH_TYPE_Q);
		nhlst=nhQDeptMonthService.getDeptMonthList(nh);
		
		nhMonth=nhQDeptMonthService.getDeptDayNight(nh);//year年白天晚上
		...
		model.put("title", year + "年" + deptNames + "用蒸汽量统计分析");
		...
		model.put("num", num);
		model.put("deptId", deptId);
		return "nhgl/analyse/gas/dept/dept_count_all";
	}
	...
	public String allYearData(ModelMap model,String start, String deptName, String fxCode, Integer deptId, String isp) throws UnsupportedEncodingException, ParseException
	{
		...
		nhlst=nhQDeptDayService.getDeptDayList(nh);
		
		deptName = (deptName==null?"":deptName);
		String title = startTime.toString("YYYY年MM月")+deptName+"用蒸汽量统计分析";
	...
	public String dayAllYear(ModelMap model,String start, String deptName, String fxCode, Integer deptId, String isp){
		...
		deptName = (deptName == null ? "" : deptName);
		String title = startTime.toString("YYYY年MM月DD日")+deptName+"用蒸汽量统计分析";
		JDateTime now = new JDateTime(new Date());
		List<NhQDeptHour> nhlst=new ArrayList<NhQDeptHour>();
		List<NhQDeptHour> nhLastlst=new ArrayList<NhQDeptHour>();
		...
		if(now.toString("YYYY-MM-DD").equals(startTime.toString("YYYY-MM-DD"))){
			...
			nhlst=nhQDeptHourService.getDayofSubGas(nh);
			...
			nhLastlst=nhQDeptHourService.getDayofSubGas(nh);
			hour=now.getHour();
		}else{
