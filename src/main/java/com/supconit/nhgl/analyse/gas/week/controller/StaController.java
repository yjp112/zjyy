package com.supconit.nhgl.analyse.gas.week.controller;
 ...
 * 
 */
@Controller("StaGasController")
@RequestMapping("nhgl/analyse/gas/week")
public class StaController {
	...
	public String index(ModelMap model, String start, String standardCode, Integer number,String areaId,String deptId) {
		...
		return "nhgl/analyse/gas/week/week_count_tab";
	}

	@RequestMapping("weekCount")
	public String weekCount(ModelMap model, String start, String deptName, String isp, Integer id, String standardCode, String number) throws UnsupportedEncodingException, ParseException {
		num = num + 1;
		deptName = (deptName == null ? "" : deptName);
		JDateTime dateTime = new JDateTime(new Date());
		JDateTime jdtime = new JDateTime(start);
		String title = jdtime.toString("YYYY年MM月") + deptName + "周用蒸汽量统计分析";
		...
		if(number.equals("1")){
			...
			nhlst=nhQDeptDayService.getDeptWeekList(nh);
			
			if(nhlst.size() > 0){
				for(NhQDeptDay n : nhlst){
			...
			nhDay=nhQDeptDayService.getDeptDayNight(nh);//这个月白天晚上
			...
			nhDayLast=nhQDeptDayService.getDeptDayNight(nh);
			...
			nhlst=nhQAreaDayService.getAreaWeekList(nh);
			
			if(nhlst.size() > 0){
				for(NhQAreaDay n : nhlst){
					work.append(n.getWorkTotal()).append(",");
					week.append(n.getWeekendTotal()).append(",");
					weekCount.append("\"" + n.getWeekName() + "\"").append(",");
				}
			...
			nhDay=nhQAreaDayService.getAreaDayNight(nh);//这个月白天晚上
			//上一年这个月白天晚上
			nh.setStartTime(GraphUtils.addYear(stime.toString("YYYY-MM-DD"), -1));
			nh.setEndTime(GraphUtils.addYear(etime.toString("YYYY-MM-DD"), -1));
			nhDayLast=nhQAreaDayService.getAreaDayNight(nh);
		...
		model.put("number", number);
		model.put("num", num);
		return "nhgl/analyse/gas/week/week_count_all";
	}
	...
	public String dayCount(ModelMap model, String first, String number, String standardCode, Integer id, String isp,String start) {
		...
		if("1".equals(number)){
			...
			nhlst=nhQDeptDayService.getDeptWeekDays(nh);
			...
			nhlst=nhQAreaDayService.getAreaWeekDays(nh);
	...
	private void weekCounts(ModelMap model, String first, String deptName, String flag, Integer id, String standardCode,String endTime){
		deptName = deptName == null ? "" : deptName;
		String titles = first + deptName + "用蒸汽量统计分析";
