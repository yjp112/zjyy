package com.supconit.nhgl.analyse.electric.area.controller;
 ...
 */
@Controller
@RequestMapping("nhgl/analyse/electric/area")
public class AreaCountController {
	...
	public String index(ModelMap model, Integer yearParam, String flag, String fxCode,String code,String areaId) {
		...
		nhlst=nhDAreaMonthService.getAreaList(nh);
		...
		return "nhgl/analyse/electric/area/area_count_tab";
	}
	...
	public String subYearData(ModelMap model, String fullLevelName, Integer year,
		...
		nh.setNhType(Constant.NH_TYPE_D);
		nhlst=nhDAreaMonthService.getAreaMonthList(nh);
		
		nhMonth=nhDAreaMonthService.getAreaDayNight(nh);//year年白天晚上
		...
		model.put("title", year + "年" + fullLevelNames + "用电量统计分析");
		...
		model.put("fxCode", fxCode);
		model.put("id", areaId);
		return "nhgl/analyse/electric/area/area_count_all";
	}
	...
	public String allYearData(ModelMap model,String start, String fullLevelName, String fxCode, Integer areaId, String isp) 
	{
		...
		nhlst=nhDAreaDayService.getAreaDayList(nh);
		
		fullLevelName = (fullLevelName==null?"":fullLevelName);
		String title = startTime.toString("YYYY年MM月")+fullLevelName+"用电量统计分析";
	...
	public String dayAllYear(ModelMap model,String start, String fullLevelName, String fxCode, Integer areaId, String isp) {
		...
		String title = startTime.toString("YYYY年MM月DD日")+fullLevelName+"用电量统计分析";
		JDateTime now = new JDateTime(new Date());
		List<NhDAreaHour> nhlst=new ArrayList<NhDAreaHour>();
		List<NhDAreaHour> nhLastlst=new ArrayList<NhDAreaHour>();
		nh.setNhType(Constant.NH_TYPE_D);
		nh.setAreaId(areaId);
		...
		if(now.toString("YYYY-MM-DD").equals(startTime.toString("YYYY-MM-DD"))){
			nh.setCollectDate(DateUtils.truncate(new Date(), Calendar.DAY_OF_MONTH));
			nh.setCollectTime(new Date());
			nhlst=nhDAreaHourService.getDayofAreaElectricty(nh);
			...
			nhLastlst=nhDAreaHourService.getDayofAreaElectricty(nh);
			hour=now.getHour();
		}else{
