package com.supconit.nhgl.analyse.energy.week;
 ...
 * 
 */
@Controller("StaEnergyController")
@RequestMapping("nhgl/analyse/energy/week")
public class StaController {
	...
	public String index(ModelMap model, String start, String standardCode, Integer number,String areaId,String deptId) {
		...
		return "nhgl/analyse/energy/week/week_count_tab";
	}
	...
	public String weekCount(ModelMap model, String start, String deptName, Integer id, String standardCode, String number) throws ParseException{
		...
		String title = jdtime.toString("YYYY年MM月") + deptName + "周用能量统计分析";
		...
		if(number.equals("1")){
			...
			nhlst=nhENDeptDayService.getDeptWeekList(nh);
			...
			nhDay=nhENDeptDayService.getDeptDayNight(nh);//这个月白天晚上
			...
			nhDayLast=nhENDeptDayService.getDeptDayNight(nh);
			...
			nhlst=nhENAreaDayService.getAreaWeekList(nh);
			...
			nhDay=nhENAreaDayService.getAreaDayNight(nh);//这个月白天晚上
			//上一年这个月白天晚上
			nh.setStartTime(GraphUtils.addYear(stime.toString("YYYY-MM-DD"), -1));
			nh.setEndTime(GraphUtils.addYear(etime.toString("YYYY-MM-DD"), -1));
			nhDayLast=nhENAreaDayService.getAreaDayNight(nh);
		...
		model.put("number", number);
		model.put("num", num);
		return "nhgl/analyse/energy/week/week_count_all";
	}
	...
	public String dayCount(ModelMap model, String first, String number, String standardCode, Integer id,String start) {
		...
		if("1".equals(number)){
			...
			nhlst=nhENDeptDayService.getDeptWeekDays(nh);
			...
			nhlst=nhENAreaDayService.getAreaWeekDays(nh);
	...
	private void weekCounts(ModelMap model, String first, String deptName, String flag, Integer id, String standardCode,String endTime){
		deptName = deptName == null ? "" : deptName;
		String titles = first + deptName + "用能量统计分析";
