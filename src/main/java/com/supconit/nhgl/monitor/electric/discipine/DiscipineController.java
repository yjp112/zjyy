package com.supconit.nhgl.monitor.electric.discipine;
...
@Controller
@RequestMapping("nhgl/monitor/electric/discipine")
public class DiscipineController {
	...
	public String index(ModelMap model, String flag) {
		...
		nhList=nhDAreaHourService.getAllSubElectricty(nh);
		...
		nhLastList = nhDAreaHourService.getAllSubElectricty(nh);
		List<NhDAreaHour> sublst = StatisticElectricUtils.getDSubList(nhList, nhLastList);
		
		model.put("subTree", sublst);
		model.put("subName", "分项");
		model.put("sTime", sTime);
		return "nhgl/monitor/electric/discipine/sub_count_tab";
	}
	...
	public String toTab(ModelMap model,String standardCode, String deptName, String eleTotal,String isParent) {
		...
		NhDAreaHour em = new NhDAreaHour();
		...
		return "nhgl/monitor/electric/discipine/sub_count_all";
	}
